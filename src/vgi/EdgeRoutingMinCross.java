...
public class EdgeRoutingMinCross extends mxGraphLayout {
	...
	protected static Set<mxICell> findRoadblocks(
		...
		for (Vertex vertex : inPath) {
			...
			}  // End if (previousLocation != null)
			previousLocation = vertex.location;

		}  // End for (Vertex vertex : inPath)

		return outRoadblocks;
	}  // protected static Set<mxICell> findRoadblocks(...)

	protected static Set<mxICell> findRoadblocks(
			Collection<List<Vertex>> inPaths,
			Set<mxICell> inPotentialRoadblocks) {
		...
		for (List<Vertex> path : inPaths) {

			outRoadblocks.addAll(EdgeRoutingMinCross.findRoadblocks(
					path,
					inPotentialRoadblocks));

		}  // End for (List<Vertex> path : inPaths)
		...
		for (List<Vertex> path : paths) {
			...
			for (Vertex aVertex : path) {
				if (aVertex.location == null) {
					throw new IllegalStateException("The 'aVertex' variable has null location.");
				}
