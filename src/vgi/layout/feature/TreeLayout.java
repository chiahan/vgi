...
public class TreeLayout extends mxGraphLayout
{
	 ...
	 * @param horizontal
	 */
	public TreeLayout(mxGraph graph, boolean horizontal)
	{
	 ...
	 * @param invert
	 */
	public TreeLayout(mxGraph graph, boolean horizontal, boolean invert)
	{
		super(graph);
	...
	public void execute(Object parent, Object root)
	{
		...
		if (root != null)
		{
			...
			try
			{
				TreeLayout.TreeNode node = dfs(root, parent, null);
	 ...
	 */
	protected void moveNode(TreeLayout.TreeNode node, double dx, double dy)
	{
		node.x += dx;
		node.y += dy;
		apply(node, null);

		TreeLayout.TreeNode child = node.child;
	 ...
	 * algorithm.
	 */
	protected TreeLayout.TreeNode dfs(Object cell, Object parent, Set<Object> visited)
	{
		if (visited == null)
		{
			...
			for (int i = 0; i < out.length; i++)
			{
				...
				if (!isEdgeIgnored(edge))
				{
					...
					Object target = graph.getView().getVisibleTerminal(edge,
							invert);
					TreeLayout.TreeNode tmp = dfs(target, parent, visited);
	 ...
	 * at the given node.
	 */
	protected void layout(TreeLayout.TreeNode node)
	{
		if (node != null)
		{
			TreeLayout.TreeNode child = node.child;
	 ...
	 * 
	 */
	protected mxRectangle horizontalLayout(TreeLayout.TreeNode node, double x0, double y0,
			mxRectangle bounds)
	{
		...
		if (child != null)
		{
			...
			TreeLayout.TreeNode s = child.next;
	 ...
	 * 
	 */
	protected void attachParent(TreeLayout.TreeNode node, double height)
	{
	 ...
	 * 
	 */
	protected void layoutLeaf(TreeLayout.TreeNode node)
	{
	 ...
	 * 
	 */
	protected double join(TreeLayout.TreeNode node)
	{
		double dist = 2 * nodeDistance;
	 ...
	 * 
	 */
	protected double merge(TreeLayout.Polygon p1, TreeLayout.Polygon p2)
	{
		double x = 0;
		double y = 0;
		double total = 0;

		TreeLayout.Polyline upper = p1.lowerHead;
		TreeLayout.Polyline lower = p2.upperHead;
		...
		if (lower != null)
		{
			TreeLayout.Polyline b = bridge(p1.upperTail, 0, 0, lower, x, y);
			p1.upperTail = (b.next != null) ? p2.upperTail : b;
			p1.lowerTail = p2.lowerTail;
		}
		else
		{
			TreeLayout.Polyline b = bridge(p2.lowerTail, x, y, upper, 0, 0);
	 ...
	 * 
	 */
	protected TreeLayout.Polyline bridge(TreeLayout.Polyline line1, double x1, double y1,
			TreeLayout.Polyline line2, double x2, double y2)
	{
		...
		TreeLayout.Polyline r = createLine(dx, dy, line2.next);
	 ...
	 * 
	 */
	protected TreeLayout.TreeNode createNode(Object cell)
	{
		TreeLayout.TreeNode node = new TreeLayout.TreeNode(cell);
	 ...
	 * 
	 */
	protected mxRectangle apply(TreeLayout.TreeNode node, mxRectangle bounds)
	{
	 ...
	 * 
	 */
	protected TreeLayout.Polyline createLine(double dx, double dy, TreeLayout.Polyline next)
	{
		return new TreeLayout.Polyline(dx, dy, next);
	}
	...
	protected static class TreeNode
	{
		 ...
		 * 
		 */
		protected TreeLayout.TreeNode child, next; // parent, sibling

		/**
		 * 
		 */
		protected TreeLayout.Polygon contour = new TreeLayout.Polygon();
	...
	protected static class Polygon
	{
		 ...
		 * 
		 */
		protected TreeLayout.Polyline lowerHead, lowerTail, upperHead, upperTail;
	...
	protected static class Polyline
	{
		 ...
		 * 
		 */
		protected TreeLayout.Polyline next;

		/**
		 * 
		 */
		protected Polyline(double dx, double dy, TreeLayout.Polyline next)
		{
			this.dx = dx;
			this.dy = dy;
			this.next = next;
