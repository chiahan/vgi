...
public class mxGraphView extends mxEventSource
{
			...
			mxCellState source, mxCellState target)
	{
		...
		//
		// For loop edges, let the edge style code handle all control points for routing.
		//
		if ((p0 == null) && (pe == null) && (source != null) && (source == target)) {
			state.getAbsolutePoints().remove(p0);
			state.getAbsolutePoints().remove(pe);
			return;
		}
